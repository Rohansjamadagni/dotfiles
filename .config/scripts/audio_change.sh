...
get_all_sinks() {
  pactl list short sinks |grep 'pci\|usb' | cut -f 2
}
...
OUT=$(echo $NEW_DEFAULT_SINK | cut -d '.' -f2 | cut -d '-' -f1)
echo $OUT
if [[ $OUT == "usb" ]]; then
  notify-send "Audio Output on: headphones"
else
  notify-send "Audio Output on: speakers"
fi
